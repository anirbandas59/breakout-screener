...
from breakout_screener.models.enums import AnalysisStatusEnum, BreakoutStatusEnum, PivotTypeEnum
...
class TestBreakoutDataRepository:
    ...
    async def test_get_by_date_range_with_symbols(
        ...
        breakout1_data = {**sample_breakout_data, "stock_id": stock1.id, "trade_date": trade_date}
        breakout2_data = {**sample_breakout_data, "stock_id": stock2.id, "trade_date": trade_date}
    ...
    async def test_get_by_advanced_filter(
        ...
        filtered_breakouts = await breakout_data_repository.get_by_advanced_filter(filters)
        
        assert len(filtered_breakouts) == 1
        assert filtered_breakouts[0].close_price == 200.0
        assert filtered_breakouts[0].breakout_status == BreakoutStatusEnum.DOWNSIDE_BREAKOUT
    ...
    async def test_get_daily_breakout_summary(
        ...
        for i in range(3):
            ...
            breakout_data.update({
                ...
                "breakout_status": BreakoutStatusEnum.UPSIDE_BREAKOUT if i < 2 else BreakoutStatusEnum.NO_BREAKOUT,
                "is_analyzed": True,
            })
            await breakout_data_repository.create(breakout_data)
