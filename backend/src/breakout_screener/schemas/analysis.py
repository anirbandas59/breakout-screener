...
class AnalysisSessionBase(BaseSchema):
    ...
    session_name: str = Field(
        ..., min_length=1, max_length=100, description="Unique session name"
    )
...
class AnalysisSessionCreate(AnalysisSessionBase):
    ...
    status: AnalysisStatusEnum = Field(
        AnalysisStatusEnum.PENDING, description="Initial status"
    )
    start_time: datetime | None = Field(None, description="Analysis start time")
...
class AnalysisSessionResponse(AnalysisSessionBase, UUIDMixin, TimestampMixin):
    ...
    is_running: bool = Field(
        False, description="Whether the session is currently running"
    )
    ...
    metrics_count: int | None = Field(
        None, ge=0, description="Number of associated performance metrics"
    )

    # Success rate
    success_rate: float | None = Field(
        None, ge=0, le=100, description="Success rate percentage"
    )
    ...
    def estimated_completion_time(self) -> datetime | None:
        ...
        if (
            self.start_time
            and self.progress_percentage
            and self.progress_percentage > 0
            and self.status == AnalysisStatusEnum.IN_PROGRESS
        ):
...
class AnalysisSessionList(ListResponse):
    ...
    items: list[AnalysisSessionResponse] = Field(
...
class AnalysisSessionFilter(FilterParams):
    ...
    # Session filters
    session_name: str | None = Field(
        None, description="Filter by session name (partial match)"
    )
    status: AnalysisStatusEnum | None = Field(None, description="Filter by status")

    # Date filters
    analysis_date_from: date | None = Field(
        None, description="Analysis date from (inclusive)"
    )
    analysis_date_to: date | None = Field(
        None, description="Analysis date to (inclusive)"
    )
    ...
    min_progress: float | None = Field(
        None, ge=0, le=100, description="Minimum progress percentage"
    )
    max_progress: float | None = Field(
        None, ge=0, le=100, description="Maximum progress percentage"
    )
...
class AnalysisSessionSummary(BaseSchema):
    ...
    total_processing_time_hours: float | None = Field(
        None, ge=0, description="Total processing time"
    )

    # Performance statistics
    total_items_processed: int | None = Field(
        None, ge=0, description="Total items processed across all sessions"
    )
    overall_success_rate: float | None = Field(
        None, ge=0, le=100, description="Overall success rate"
    )
...
class PerformanceMetricsBase(BaseSchema):
    ...
    metric_unit: str | None = Field(
        None, max_length=20, description="Unit of measurement"
    )
    metric_description: str | None = Field(
        None, max_length=200, description="Metric description"
    )

    # Context information
    context_data: dict | None = Field(
        None, description="Additional context for the metric"
    )
...
class PerformanceMetricsUpdate(BaseSchema):
    ...
    # Allow partial updates
    metric_type: PerformanceMetricTypeEnum | None = Field(
        None, description="Metric type"
    )
...
class PerformanceMetricsResponse(PerformanceMetricsBase, UUIDMixin, TimestampMixin):
    ...
    is_latest: bool = Field(
        False, description="Whether this is the latest metric of its type"
    )
...
class PerformanceMetricsList(ListResponse):
    ...
    items: list[PerformanceMetricsResponse] = Field(
...
class PerformanceMetricsFilter(FilterParams):
    ...
    metric_date_from: date | None = Field(
        None, description="Metric date from (inclusive)"
    )
...
class PerformanceMetricsSummary(BaseSchema):
    ...
    unique_metric_types: int = Field(
        ..., ge=0, description="Number of unique metric types"
    )
    ...
    recent_trend: list[dict] | None = Field(
        None, description="Recent metric trend data"
    )
...
class PerformanceMetricsBulkCreate(BaseSchema):
    ...
    def validate_metrics(
        cls, v: list[PerformanceMetricsCreate]
    ) -> list[PerformanceMetricsCreate]:
...
class MetricTrendAnalysis(BaseSchema):
    ...
    trend_strength: float | None = Field(
        None, ge=0, le=1, description="Trend strength (0-1)"
    )

    # Statistical data
    linear_regression: dict | None = Field(
        None, description="Linear regression analysis"
    )
    moving_averages: dict | None = Field(
        None, description="Moving average calculations"
    )
    volatility: float | None = Field(None, ge=0, description="Metric volatility")

    # Data points
    data_points: list[dict] = Field(
        ..., description="Actual data points used in analysis"
    )
