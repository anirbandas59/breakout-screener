...
class MasterBreakoutDataV2(BaseModel):
    ...
    # Analysis date
    trade_date = Column(
        Date, nullable=False, index=True, comment="Date of the trading session"
    )

    # OHLCV data (identical structure to BreakoutDataV2)
    open_price = Column(
        DECIMAL(12, 2), nullable=False, comment="Opening price for the trading session"
    )
    ...
    close_price = Column(
        DECIMAL(12, 2), nullable=False, comment="Closing price for the trading session"
    )
    ...
    volume = Column(
        BigInteger, nullable=False, comment="Trading volume for the session"
    )

    # CPR (Central Pivot Range) calculations
    cpr = Column(
        DECIMAL(12, 2), nullable=True, comment="Central Pivot Point: (H+L+C)/3"
    )

    resistance_1 = Column(
        DECIMAL(12, 2), nullable=True, comment="First resistance level: (2*Pivot) - Low"
    )
    ...
    support_1 = Column(
        DECIMAL(12, 2), nullable=True, comment="First support level: (2*Pivot) - High"
    )

    support_2 = Column(
        DECIMAL(12, 2), nullable=True, comment="Second support level: Pivot - (R1 - S1)"
    )

    # Analysis indicators
    narrow_gap = Column(
        Boolean, default=False, nullable=False, comment="Whether CPR shows narrow gap"
    )
    ...
    # Analysis metadata
    chart_link = Column(Text, nullable=True, comment="URL link to chart analysis")

    analysis_notes = Column(
        Text, nullable=True, comment="Additional analysis notes and observations"
    )

    confidence_score = Column(
        DECIMAL(3, 2), nullable=True, comment="Analysis confidence score (0.0 to 1.0)"
    )
    ...
    # Relationships
    stock = relationship("Stock", back_populates="master_breakout_data", lazy="joined")
    ...
    __table_args__ = (
        ...
        Index(
            "idx_master_breakout_data_v2_breakout_snapshot",
            "breakout_indicator",
            "snapshot_date",
        ),
        {
    ...
    def __repr__(self) -> str:
        return (
            f"<MasterBreakoutDataV2(stock_id={self.stock_id}, trade_date={self.trade_date}, "
            f"snapshot_date={self.snapshot_date})>"
        )

    @classmethod
    def from_breakout_data(
        cls, breakout_data: "BreakoutDataV2", snapshot_date: datetime | None = None
    ...
    @classmethod
    def from_v1_master_data(
        cls, v1_record: dict, stock_id: str
    ) -> "MasterBreakoutDataV2":
        """
    ...
    def to_dict(
        self, include_stock: bool = True, exclude_fields: list | None = None
    ) -> dict:
        """
        ...
        result["price_change_percent"] = float(
            self.calculate_price_change_percent() or 0
        )
